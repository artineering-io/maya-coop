    ...
    for shape in shapes:
        ...
        for color_set in color_sets:
            if color_set not in shape_color_sets:
                ...
                vtx_colors = fn_mesh.getVertexColors(
                    colorSet=color_set, defaultUnsetColor=om.MColor((0, 0, 0, 0)))
...
def create_color_set(shapes, color_sets, delete_history=True):
    ...
    for shape in shapes:
        ...
        for color_set in color_sets:
            if color_set not in shape_color_sets:
                cmds.polyColorSet(shape, cs=color_set,
                                  representation="RGBA", create=True)
                # needs the object selected
                cmds.polyColorPerVertex(rgb=(0.0, 0.0, 0.0), a=0.0)
...
def delete_color_sets(shapes, color_sets, retry=False):
    ...
    color_sets_set = set(color_sets)  # O(1) lookup
    nodes2delete = []
    ...
    for shape in shapes:
        ...
        for node in history:
            if cmds.objectType(node) == "polyColorPerVertex":
                ...
                # get representation
                representation = cmds.getAttr(
                    "{0}.representation".format(node))
                ...
                # get vertex colors
                alphas = cmds.getAttr(
                    "{0}.vertexColor[*].vertexFaceColor[*].vertexFaceAlpha".format(node))
                reds = cmds.getAttr(
                    "{0}.vertexColor[*].vertexFaceColor[*].vertexFaceColorR".format(node))
                greens = cmds.getAttr(
                    "{0}.vertexColor[*].vertexFaceColor[*].vertexFaceColorG".format(node))
                blues = cmds.getAttr(
                    "{0}.vertexColor[*].vertexFaceColor[*].vertexFaceColorB".format(node))
                print(alphas)
                print(reds)
                print(greens)
                print(blues)
                ...
                color_sets = cmds.getAttr(
                    "{0}.colorSet[*].colorName".format(shape))
                ...
                clib.set_attr(shape, "colorSet[{0}].colorName".format(
                    color_set_index), color_set_name)
                clib.set_attr(shape, "colorSet[{0}].representation".format(
                    color_set_index), representation)
                clib.set_attr(shape, "colorSet[{0}].clamped".format(
                    color_set_index), clamped)
                for i in range(len(alphas)):
                    """
                    attr = "colorSet[{0}].colorSetPoints[{1}]".format(colorSetIndex, i)
                    setAttr(shape, attr, [reds[i], greens[i], blues[i], alphas[i]])
                    """
                    attr = "colorSet[{0}].colorSetPoints[{1}].colorSetPointsR".format(
                        color_set_index, i)
                    # setAttr(shape, attr, reds[i])
                    cmds.setAttr("{0}.{1}".format(shape, attr), reds[i])
                    attr = "colorSet[{0}].colorSetPoints[{1}].colorSetPointsG".format(
                        color_set_index, i)
                    # setAttr(shape, attr, greens[i])
                    cmds.setAttr("{0}.{1}".format(shape, attr), greens[i])
                    attr = "colorSet[{0}].colorSetPoints[{1}].colorSetPointsB".format(
                        color_set_index, i)
                    # setAttr(shape, attr, blues[i])
                    cmds.setAttr("{0}.{1}".format(shape, attr), blues[i])
                    attr = "colorSet[{0}].colorSetPoints[{1}].colorSetPointsA".format(
                        color_set_index, i)
                    # setAttr(shape, attr, alphas[i])
                    cmds.setAttr("{0}.{1}".format(shape, attr), alphas[i])
                # add to shape
                face_no = cmds.polyEvaluate(
                    shape, face=True)  # query amount of faces
                mel_cmd = 'setAttr -s {0} "{1}.fc[0:{2}]" -type "polyFaces"'.format(
                    face_no, shape, face_no - 1)
                ...
                for f in range(face_no):
                    ...
                    v_string = cmds.polyInfo(
                        "{0}.f[{1}]".format(shape, f), fv=True)[0]
                    vertices = v_string.split(':')[1].split()
                    mel_cmd += ' mc {0} {1}'.format(
                        color_set_index, len(vertices))
                    for _ in vertices:
                        mel_cmd += " {0}".format(fv)
                        fv += 1
                mel_cmd += ';'
                ...
                cmds.delete(node)
                LOG.debug("Vertex color set {0} baked on {1}".format(
                    color_set_name, shape))
